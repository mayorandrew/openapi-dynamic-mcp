...
export async function loadConfig(configPath: string): Promise<RootConfig> {
  ...
  try {
    ...
    throw new OpenApiMcpError("CONFIG_ERROR", `Cannot read config file: ${configPath}`, {
  ...
  try {
    ...
    throw new OpenApiMcpError("CONFIG_ERROR", `Invalid YAML in config: ${configPath}`, {
      cause: error instanceof Error ? error.message : String(error)
    });
  }
  ...
  for (const api of parsed.data.apis) {
    ...
    if (seenNames.has(normalized)) {
      throw new OpenApiMcpError("CONFIG_ERROR", `Duplicate API name: ${api.name}`);
    }
    ...
    try {
      ...
      throw new OpenApiMcpError("CONFIG_ERROR", `OpenAPI schema file not found: ${resolvedSpecPath}`, {
        apiName: api.name
      });
    }
