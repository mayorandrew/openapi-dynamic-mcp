...
describe("executeEndpointRequest", () => {
  it("merges headers and returns JSON response", async () => {
    ...
    nock("https://override.example.com")
      ...
      .reply(200, { items: [{ id: 1 }] }, { "content-type": "application/json" });
  ...
  it("returns binary fallback for non-text content", async () => {
    ...
    expect(result.response.bodyBase64).toBe(Buffer.from([1, 2, 3, 4]).toString("base64"));
  });
  ...
  it("respects Retry-After when present", async () => {
    ...
    nock("https://api.example.com")
      .get("/v1/pets")
      .query(true)
      .reply(429, { error: "rate_limited" }, { "content-type": "application/json", "retry-after": "1" })
      .get("/v1/pets")
      .query(true)
      .reply(200, { ok: true }, { "content-type": "application/json" });
  ...
  it("falls back to exponential backoff on invalid Retry-After", async () => {
    ...
    const scope = nock("https://api.example.com")
      .get("/v1/pets")
      .query(true)
      .reply(429, { error: "rate_limited" }, { "content-type": "application/json", "retry-after": "invalid" })
      .get("/v1/pets")
      .query(true)
      .reply(200, { ok: true }, { "content-type": "application/json" });
