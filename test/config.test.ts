...
describe("loadConfig", () => {
  it("loads and resolves spec paths", async () => {
    ...
    expect(config.apis[0].specPath).toBe(path.join(fixturesDir, "pet-api.yaml"));
  ...
  it("fails on duplicate names", async () => {
    await expect(loadConfig(path.join(fixturesDir, "config-duplicate-names.yaml"))).rejects.toMatchObject({
      code: "CONFIG_ERROR"
    } satisfies Partial<OpenApiMcpError>);
  });

  it("fails on missing spec file", async () => {
    await expect(loadConfig(path.join(fixturesDir, "config-missing-spec.yaml"))).rejects.toMatchObject({
      code: "CONFIG_ERROR"
    } satisfies Partial<OpenApiMcpError>);
  });

  it("fails on invalid retry429 config", async () => {
    await expect(loadConfig(path.join(fixturesDir, "config-invalid-retry.yaml"))).rejects.toMatchObject({
      code: "CONFIG_ERROR"
    } satisfies Partial<OpenApiMcpError>);
  });
});
